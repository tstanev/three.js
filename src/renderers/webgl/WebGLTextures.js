...
function WebGLTextures( _gl, extensions, state, properties, capabilities, paramThreeToGL, infoMemory ) {
	...
	function paramToInternal( format, type, isDepth ) {
		...
		} else if ( format === THREE.DepthStencilFormat ) {

			switch ( type ) {

				case THREE.UnsignedInt248Type : return _gl.DEPTH24_STENCIL8;
				default: break;

			}

		} else if ( format === THREE.DepthFormat ) {
	...
	function uploadTexture( textureProperties, texture, slot ) {
		...
		if ( texture.isDepthTexture ) {
			...
			//TODO: This logic duplicates stuff that is better done in paramToInternal
			//together with all the other mappings. 
			...
			if ( texture.format === DepthStencilFormat ) {
				...
				internalFormat = _gl.DEPTH24_STENCIL8;
		...
		if ( textureNeedsGenerateMipmaps( texture ) ) _gl.generateMipmap( _gl.TEXTURE_2D );
	...
	function setupFrameBufferTexture( framebuffer, renderTarget, attachment, textureTarget ) {
		...
		var glInternalFormat = paramToInternal( renderTarget.texture.format, renderTarget.texture.type );
